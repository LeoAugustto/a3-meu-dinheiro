...
import { BarChart3, ChevronLeft, ChevronRight, CreditCard, Landmark, LayoutDashboard, LogOut, Settings, Tags, Target, UserRound, WalletCards } from "lucide-react";
import logo from "../assets/logo-meu-dinheiro-v2.png";
...
function Sidebar({ collapsed, onToggle, onLogout }) {
  ...
  return (
    <aside className={`sidebar ${collapsed ? "is-collapsed" : ""}`}>
      ...
      <button className="sidebar-toggle" type="button" title={toggleLabel} aria-label={toggleLabel} onClick={onToggle}>
      ...
      <nav className="nav-menu" aria-label="Menu principal">
        {menuItems.map((item) => {
          ...
          return (
            <NavLink key={item.to} to={item.to} end={item.to === "/"} className={({ isActive }) => (isActive ? "nav-link is-active" : "nav-link")} aria-label={item.label} title={item.label}>
              <Icon size={19} />
